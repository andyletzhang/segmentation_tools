...
@@ scipy.interpolate.fitpack is now deprecated. Redirected the imports to scipy.interpolate
"""
...
class Contour(PointSet):
    ...
    def spline_derivatives(self, begin, end, derivatives=1):
        ...
        ret = [numpy.transpose(splev(points, tck, der=d)) for d in derivatives]
        if unpack:
            ret = ret[0]
        return ret
    ...
    def inward_normals(self, positions=None):
        ...
        first_der = numpy.transpose(splev(positions, tck, 1))
    ...
    def interpolate_points(self, positions):
        ...
        from scipy.interpolate import splev
        tck, uout = self.to_spline()
        return numpy.transpose(splev(positions, tck))
...
#            self.points[-1] = self.points[0]
    ...
    def to_spline(self, smoothing = 0, spacing_corrected = False):
        ...
        points[-1] = points[0]
        tck, uout = splprep(x = points.transpose(), u = u, per = True, s = smoothing)
        return tck, uout
...
class CentralAxisContour(Contour):
    ...
    def from_contour(cls, contour, start, end, num_points, scale_steps=5, torsion_step=0.001,
        ...
        start_pos, end_pos = numpy.transpose(splev([start, end], tck))
        ...
        for points in scale_steps:
            ...
            new_axis = numpy.transpose(splev(numpy.linspace(0, uout[-1], points, endpoint=True), tck))
    ...
    def recalculate_central_axis(self, tck=None):
        ...
        spatial_pos = numpy.transpose(splev(self.axis_positions, tck))
    ...
    def axis_spline_derivatives(self, begin=None, end=None, derivatives=1):
        ...
        ret = [numpy.transpose(splev(list(range(begin, end)), tck, der=d)) for d in derivatives]
        if unpack:
            ret = ret[0]
        return ret
    ...
    def axis_to_spline(self, smoothing = 0, spacing_corrected = False, end_weight = None):
        ...
        if end_weight is not None:
            ...
            weights = None
        tck, uout = splprep(x = self.central_axis.transpose(), u = u, s = smoothing, w = weights)
        return tck, uout
    ...
    def axis_top_bottom_to_spline(self):
        ...
        top_tck = splrep(position_vals, top)
        bottom_tck = splrep(position_vals, bottom)
    ...
    def resample_axis(self, num_points):
        ...
        top_p = splev(positions, top_tck) % contour_points
        bottom_p = splev(positions, bottom_tck) % contour_points
