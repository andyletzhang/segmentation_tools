...
def read_nd2_file(file_path, progress_bar=None, **progress_kwargs):
    if progress_bar is None:
        progress_bar = lambda x, **kwargs: x
    with ND2File(file_path) as file:
        shape=read_nd2_shape(file) # (T, Z, C, Y, X)
        shape_dialog = ND2ShapeDialog(shape) # Prompt user to select ranges to import for each dimension
        ...
        nd2_file=read_nd2(file) # Load the ND2 file
        sliced=nd2_file[np.ix_(t_bounds, z_bounds)] # index the T and Z dimensions
        img_shape=(shape[3], shape[4], len(c_bounds)) # output image shape (Y, X, C)

        # Assuming `sliced_array` is the array of functions
        result_array = np.empty((*sliced.shape, *img_shape), dtype=np.int16)  # Initialize output array

        # Read the data from the ND2 file
