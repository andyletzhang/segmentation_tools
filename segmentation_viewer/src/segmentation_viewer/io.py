...
def read_image_file(file_path, progress_bar=None, image_shape=None, **progress_kwargs):
    ...
    if image_shape is not None:
        if image_shape == "all":
            t_bounds, p_bounds, z_bounds, c_bounds = (
                slice(None),
                slice(None),
                slice(None),
                slice(None),
        ...
        shape_dialog = ShapeDialog(
            shape
        )  # Prompt user to select ranges to import for each dimension
    ...
    sliced = placeholder[
        np.ix_(t_bounds, p_bounds, z_bounds)
    ]  # index the P, T, and Z dimensions
    ...
    result_array = np.empty(
        (*sliced.shape, *img_shape), dtype=np.uint16
    )  # Initialize output array

    # Read the data from the file
    for idx, get_img in progress_bar(
        np.ndenumerate(sliced), length=sliced.size, **progress_kwargs
    ):
        img = get_img()
        if img.ndim == 2:  # mono
            img = img[np.newaxis]
        result_array[idx] = img.transpose(1, 2, 0)[
            ..., c_bounds
        ...
        try:
            ...
            if len(slices[0]) > 1 and len(slices[1]) > 1:
                QMessageBox.warning(
                    self,
                    "Invalid Input",
                    "Currently, either P or T dimension can be loaded, but not both.",
                )
                return None
...
class ExportWizard(QDialog):
    def __init__(self, dataframe, total_rows, parent=None, root_path=""):
        ...
        self.table_preview = QTableWidget(
            self.preview_rows + 2, len(self.attributes), self
        )
        self.populate_table_preview()
        self.table_preview.resizeColumnsToContents()
        for col in range(len(self.attributes)):
            column_width = max(
                self.table_preview.columnWidth(col) + 10, self.minimumColumnWidth
            )
    ...
    def populate_table_preview(self):
        ...
        for col, attribute in enumerate(self.attributes):
            ...
            checkbox.clicked.connect(
                lambda state, col=col: self.toggle_column_visibility(col, state)
            )
            self.checkboxes[attribute] = (
                checkbox  # Store reference to checkbox for later access
            )
            header_layout.addWidget(checkbox)

            # Set the widget to the header
            self.table_preview.setCellWidget(
                0, col, header_widget
            )  # Column-index base `wiring`
        ...
        for row in range(self.preview_rows):
            self.table_preview.setVerticalHeaderItem(
                row + 1, QTableWidgetItem(f"{row}")
            )
            for col in range(len(self.attributes)):
                try:
                ...
                cell_item.setFlags(
                    cell_item.flags() & ~Qt.ItemFlag.ItemIsEditable
                )  # Disable editing
                self.table_preview.setItem(row + 1, col, cell_item)

        self.table_preview.setVerticalHeaderItem(
            self.preview_rows + 1, QTableWidgetItem("...")
        )
        for col in range(len(self.attributes)):
    ...
    def confirm(self):
        ...
        if not self.save_path:
            QMessageBox.warning(
                self, "Validation Error", "Please specify a save location."
            )
            return
        ...
        if not self.checked_attributes:
            QMessageBox.warning(
                self, "Validation Error", "Please select at least one column."
            )
            return
